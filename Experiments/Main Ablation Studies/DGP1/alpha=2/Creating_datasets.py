...
def save_simulation_data(n, num_simulations=100, mode=1):
    for i in range(num_simulations):
        y, X, w, tau, b, e, ate = simulate_data(mode=mode, n=n)
        ...
        # Save to CSV
        filename = f'dataset_{i+1}_N{n}.csv'
        df.to_csv(filename, index=False)

# Define parameters
sample_sizes = [250]

# Generate datasets for each sample size
for N in sample_sizes:
    save_simulation_data(n=N, num_simulations=100, mode=5)